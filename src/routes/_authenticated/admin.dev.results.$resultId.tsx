...
export const Route = createFileRoute("/_authenticated/admin/dev/results/$resultId")({
...
function DrillDetail() {
  ...
  const { data: r } = useQuery({
    queryKey: ["dev-result", resultId],
    queryFn: async () => {
      const { data, error } = await supabase.from("results").select("*").eq("id", resultId).maybeSingle();
  ...
  if (!r) {
    return (
      <>
        <PageHeader title="Drill" subtitle="" />
  ...
  return (
    <>
      <PageHeader title={(r.drill_name as string) ?? "Drill"} subtitle={fmtDT(r.played_at as string)} />
      <main className="mx-auto -mt-4 max-w-md space-y-4 px-5 pb-8">
        ...
        <div className="grid grid-cols-3 gap-2">
          <Stat label="Score" value={`${r.score}/${(r.max_score as number | null) ?? "—"}`} />
          <Stat label="BSI" value={Number(r.bsi).toFixed(1)} />
          <Stat label="Bowls" value={bowls.length} />
        </div>

        <div className="rounded-2xl bg-card p-3 text-xs bt-shadow-elevated">
          <p><b>Category:</b> {(r.category as string) ?? "—"}</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead className="text-muted-foreground">
                <tr><th className="text-left">End</th><th>Bowl</th><th>Hand</th><th>Pts</th><th>x</th><th>y</th></tr>
              </thead>
              <tbody>
                {bowls.map((b, i) => (
